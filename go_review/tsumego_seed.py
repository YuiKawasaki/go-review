...
def _build_ld(
    ...
    この形（白の隣接点だけでなく内側の空き地の隣接点からも黒を作る）は
    盤のどこに置いても check_ld_shape の「漏れ」検査は通る。ただし、
    形によっては盤端の代わりに置かれた黒が1〜2個だけ孤立し、それが
    たまたま急所に接して局面をねじ曲げることがある（straight-three /
    bent-three を横へずらして試したところ、32件中32件が「差が出ない」
    形になって不採択だった。t-four は同じやり方で問題なく、12件とも
    採択された）。孤立黒を白側で塞ぐ案も試したが、塞いだ白石自身が
    孤立して1呼吸点になり、別の不備を生んだ。

    結論: この関数の作り方自体は残すが、"ずらして水増しできる形" は
    ここでは決め打ちにせず、必ず実測（KataGo の採否）で確かめる。
    """
...
# 眼形不足（初心者向け・目安10級）。
#
# 「丁字四」は、囲碁で最初に習う"殺せる眼形"の代表。同じ急所の考え方を
# 盤のいろいろな位置に置き直して繰り返し練習できるようにする。手を
# 抜いた水増しではなく、初心者向けの本が同じ形を場所を変えて何度も
# 出すのと同じ理由（置かれた場所が変わっても急所の見つけ方は同じ、
# というパターン認識の練習）。
#
# 「直三」「曲がり三目」も横へずらせば同じように増やせるはずだったが、
# 実際に KataGo で確かめると 32 件中 32 件が不採択だった。ずらすと
# 盤の左端が担っていた"壁の代わり"が無くなり、それを埋めるために
# 生じる黒の断片がたまたま急所に接して、局面をねじ曲げてしまうため
# （このファイルの _build_ld のコメント参照）。white 側で塞ぐ案も
# 試したが、塞いだ白石自身が孤立して同じ問題を起こした。
# → 教訓: 「構造として閉じている（check_ld_shape が通る）」ことと
#   「詰碁として成立する（KataGo が決め打てる）」ことは別。
#   ずらして増やせるかどうかは、必ず少数を実際に検証してから広げる。
#
# 丁字四は実測で 12/12（4位置 × 4対称）が採択されたので信頼できる。
# ここでは残りの位置（dx=0 は既存の t-four と重複するので使わない）と、
# 同じく実測で通っている「5目中手（ふくらみ）」の横シフトを試す。
...
_EYE_SHAPE_BASES: list[
    ...
    ("bulky5", [(3, 0), (3, 1), (2, 1), (2, 2), (1, 2), (0, 2)],
     [(0, 0), (1, 0), (2, 0), (0, 1), (1, 1)], [1, 2, 3]),
]
